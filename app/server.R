...
server <- function(input, output, session) {
  ...
  # EARLY setup: determine case study and apply path mapping immediately (before any reads)
  {
      ...
      resolve_paths <- function() {
        ...
        # fallback to legacy flat folders if nested not found
      ...
      # Expose common variables locally (override globals from global.R safely per-session)
      save_dir   <- DATA_PATH
      input_dir  <- INPUT_PATH
      output_dir <- OUTPUT_PATH
      pareto_path <- file.path(DATA_PATH, "pareto_fitness.txt")

      # Transparent path mapping so existing '../data|input|output' usages work per session
      path_map <- function(p) {
      ...
      # Make mapper/paths visible to helpers defined outside this environment
      ...
      # Map ini read/write so config.ini is per-session
      read.ini  <- function(file, ...) ini::read.ini(path_map(file), ...)
      write.ini <- function(x, file, ...) ini::write.ini(x, path_map(file), ...)

      # Also map common directory utilities that may be used with ../data paths
      ...
      read_sf <- function(...) {
        ...
        lyr <- if (!is.null(args$layer)) as.character(args$layer) else NA_character_
        message(sprintf("sf::read_sf dsn=%s layer=%s", as.character(args$dsn), lyr))
      ...
      st_read <- function(...) {
        ...
        if (!is.null(args$dsn)) args$dsn <- path_map(args$dsn)
        lyr <- if (!is.null(args$layer)) as.character(args$layer) else NA_character_
        message(sprintf("sf::st_read dsn=%s layer=%s", as.character(args$dsn), lyr))
        do.call(sf::st_read, args)
      }

      # Wrap selected helper functions to map path-like arguments if they are provided
      ...
      if (exists("cmf", mode = "function")) {
        .orig_cmf <- cmf
        cmf <- function(...){
          args <- list(...)
          if (!is.null(args$hru_in_opt_path)) args$hru_in_opt_path <- path_map(args$hru_in_opt_path)
          do.call(.orig_cmf, args)
        }
      }
  }

  # Session-scoped case study selection without touching filesystem (supports many concurrent tabs)
  # IMPORTANT: perform resolution and overrides immediately so early reads use the correct CS paths
  observeEvent(TRUE, {
      qs <- tryCatch(parseQueryString(isolate(session$clientData$url_search)), error = function(e) NULL)
      requested_cs <- if (!is.null(qs)) qs[["cs"]] else NULL
      base_dir <- normalizePath("..", winslash = "/", mustWork = TRUE)

    # Helper to resolve a folder for this session
    # Resolve case folder and nested paths (prefer '<cs>/{data,input,output}', fallback to legacy and default)
    resolve_paths <- function() {
      case_folder_candidates <- character(0)
      if (!is.null(requested_cs) && nzchar(requested_cs)) {
    ...
    # Expose mapper/paths to globally defined helpers via options (session-scoped)
    options(wp5_path_map = path_map,
            wp5_paths = list(data = DATA_PATH, input = INPUT_PATH, output = OUTPUT_PATH))

    # Shadow common I/O to apply path_map automatically within this session
    file.exists <- function(paths) base::file.exists(path_map(paths))
    read.table  <- function(file, ...) utils::read.table(path_map(file), ...)
    read.csv    <- function(file, ...) utils::read.csv(path_map(file), ...)
    readRDS     <- function(file, ...) base::readRDS(path_map(file), ...)
    ...
    read_sf <- function(...) {
      args <- list(...)
      if (!is.null(args$dsn)) args$dsn <- path_map(args$dsn)
      # lightweight trace for troubleshooting
      lyr <- if (!is.null(args$layer)) as.character(args$layer) else NA_character_
      message(sprintf("sf::read_sf dsn=%s layer=%s", as.character(args$dsn), lyr))
      do.call(sf::read_sf, args)
    }
    ...
    # Info banner (show repo-relative paths for clarity)
    relp <- function(p) {
    ...
    info_html <- paste0(
      ...
      " | data=", relp(DATA_PATH),
      " | input=", relp(INPUT_PATH),
      " | output=", relp(OUTPUT_PATH)
    )
  ...
  #pull status quo (robust to missing/unreadable file)
  observe({
    path_sq <- "../data/sq_fitness.txt"
    if (!file.exists(path_sq)) {
      return(invisible(NULL))
    }
    req(objectives())
    # Determine delimiter safely
    sep <- tryCatch(deli(path_sq), error = function(e) ',')
    # Read table safely; if it fails, do nothing instead of crashing
    st_q <- tryCatch(
      read.table(path_sq, header = FALSE, stringsAsFactors = FALSE, sep = sep),
      error = function(e) NULL
    )
    # Normalize to one-row data.frame with correct number of columns if possible
    normalize_sq <- function(x) {
      # Return NULL if cannot be normalized to expected length
      if (is.null(x)) return(NULL)
      expected <- length(objectives())
      # If already a data.frame with expected columns, keep
      if (is.data.frame(x) && ncol(x) == expected) return(x)
      # If data.frame with a single column, try to split the first cell
      if (is.data.frame(x) && ncol(x) == 1) {
        cell <- as.character(x[[1]][1])
      } else if (!is.data.frame(x)) {
        # vector or other: collapse to a single string
        cell <- paste0(as.character(x), collapse = " ")
      } else {
        cell <- NULL
      }
      if (is.null(cell) || is.na(cell) || !nzchar(cell)) return(NULL)
      # Try multiple delimiters
      parts <- unlist(strsplit(cell, "[ ,;\t]+"))
      nums <- suppressWarnings(as.numeric(parts))
      nums <- nums[!is.na(nums)]
      if (length(nums) == expected) {
        df <- as.data.frame(as.list(nums), stringsAsFactors = FALSE)
        names(df) <- paste0("V", seq_len(expected))
        return(df)
      }
      NULL
    }
    st_q2 <- normalize_sq(st_q)
    if (!is.null(st_q2)) {
      names(st_q2) <- objectives()
      stq(st_q2)
    } else if (!is.null(st_q) && is.data.frame(st_q) && ncol(st_q) == length(objectives())) {
      # Fallback: original parsed correctly but we got here due to some edge case
      names(st_q) <- objectives()
      stq(st_q)
    } else {
      # Log a helpful message and skip setting stq to avoid crash
      message("sq_fitness.txt could not be parsed into ", length(objectives()), " numeric values; skipping status quo load.")
    }
  })
  ...
  observeEvent(input$tabs == "data_prep",{
    ...
    output$obj_conf <- renderTable({
      ...
      if (is.null(rng) || nrow(rng) == 0) {
        return(data.frame(objective = character(0), min = numeric(0), max = numeric(0), stringsAsFactors = FALSE))
      }
      bng = rng
    ...
    observeEvent(input$files_avail,{
      ...
      output$fileStatusMessage <- renderText({
        status_html <- if (all(checkFiles) & file.exists("../input/object_names.RDS")) {
          "All Files found."
          ...
          paste("The following file(s) are missing:<br/>", paste(sub('../data/', '', missing_files), collapse = "<br/> "))
        }
        HTML(paste0("<div style='margin-bottom:8px;'>", app_dir_info(), "</div>", status_html))
      })
   ...
   observe({
    ...
    output$download_shp <- downloadHandler(
      ...
      content = function(file) {
        ...
        shp_files <- c(paste0(shp_base, ".shp"), paste0(shp_base, ".shx"), paste0(shp_base, ".dbf"), paste0(shp_base, ".prj"))

        # Create zip directly to the provided file path
  ...
  output$whole_range <- renderTable({
    ...
    df <- mima_fun(fit())
    # Ensure df is a proper data.frame (avoid dplyr::mutate on a list)
    df <- tryCatch(as.data.frame(df, stringsAsFactors = FALSE), error = function(e) {
      warning(sprintf("whole_range: coercion to data.frame failed: %s", e$message))
      as.data.frame(list())
    })

    # Pretty rounding for numeric columns
    df <- df %>%
      mutate(across(where(is.numeric), ~ if_else(abs(.) < 1, round(., 4), if_else(abs(.) < 10, round(., 2), round(., 0)))))

    # Only transform columns that exist in df
    target_cols <- intersect(cols, colnames(df))
    if (length(target_cols) > 0) {
      df <- df %>% mutate(across(all_of(target_cols), ~ {
        has_positive <- any(. > 0)
        has_negative <- any(. < 0)
    ...
    # Convert numeric columns to character for display
    df <- df %>% mutate(across(where(is.numeric), ~ as.character(.)))

    # Safely apply column names to avoid length mismatch errors
    if (!is.null(colname_unit)) {
      # Number of names we can safely assign
      k <- min(ncol(df), length(colname_unit))
      if (k > 0) {
        colnames(df)[seq_len(k)] <- colname_unit[seq_len(k)]
      }
      # If there are more data columns than provided names, generate fallbacks
      if (ncol(df) > length(colname_unit)) {
        extra_n <- ncol(df) - length(colname_unit)
        fallback <- paste0("objective", seq_len(extra_n) + length(colname_unit))
        colnames(df)[(length(colname_unit) + 1):ncol(df)] <- fallback
      }
      # Emit a lightweight warning to aid diagnostics when lengths differ
      if (length(colname_unit) != ncol(df)) {
        warning(sprintf("whole_range: names length (%d) != ncol(df) (%d); applied partial naming.", length(colname_unit), ncol(df)))
      }
    }
    df
  }, rownames = TRUE)
  ...
  observe({
    ...
    if (all(file.exists(map_files))) {
      ...
      if(!is.null(buff_els)){
        ...
        # Guard against missing catchment map (cm) or empty buffer set
        if(!is.null(cm()) && nrow(hru_ever_buffer) > 0){
          bc = cm() %>% dplyr::filter(id %in% hru_ever_buffer$id) %>% sf::st_make_valid()
          relda_utm = sf::st_transform(bc, crs = 32633) # UTM zone 33N
          buffy <- sf::st_buffer(relda_utm, dist = 60)
          buffers(sf::st_transform(buffy, crs = sf::st_crs(bc))) # all buffers ever required
        } else {
          buffers(NULL)
        }
      }else{buffers(NULL)}
  ...
  output$download_freq <- downloadHandler(
    ...
    content = function(file) {
      ...
      if (is.null(freqmap)) {
        ...
        stop("Aucune carte n'est disponible pour l'export PNG (sélection vide). Veuillez faire une sélection puis réessayer.")
      }
