# K-medoids clustering with PCA and outlier handling in R
library(cluster)
library(ggplot2)
library(gridExtra)
library(reshape2)
...
remove_outliers <- function(data, deviations, count) {
  # Calculate z-scores
  z_scores <- scale(data)
  
  # Find outliers (points where 'count' or more variables are beyond 'deviations' std devs)
  outliers <- abs(z_scores) > deviations
...
kmedoids_clustering <- function(pca_data, input_data, num_clusters) {
  # Use PAM (Partitioning Around Medoids) for k-medoids clustering
  pam_result <- pam(pca_data, k = num_clusters, diss = FALSE, metric = "euclidean")
  
  # Get medoid indices in the original input_data
...
# Function to run k-medoids multiple times and find best solution
...
non_integer_range <- function(start, stop, step, precision = 5) {
...
main <- function() {
  ...
  if (handle_outliers_boolean) {
      ...
      for (d in non_integer_range(deviations_min, deviations_max, deviations_step)) {
        for (c in count_min:count_max) {
          ...
          if (data_hash_key %in% names(cache)) {
            ...
            cat(sprintf("Best Cluster Count: %d, Best PCA Silhouette Score: %.4f\n", 
            ...
            cat(sprintf("\nnumber of extreme solutions: %d, deviations: %.2f, count: %d\n", 
                        num_outliers, d, c))
            clustering_result <- run_kmedoids_multiple_times(pca_data, min_clusters, max_clusters, 
    ...
    # Export to CSV
    out_file_path <- file.path(output_path, "kmedoid_data_w_clusters_representativesolutions_outliers.csv")
    ...
    # Export to CSV
    out_file_path <- file.path(output_path, "kmedoid_data_w_clusters_representativesolutions.csv")
  ...
  if (num_variables_to_plot == 2) {
    p <- ggplot(rep_data, aes_string(x = var_1, y = var_2)) +
      ...
      labs(title = "Representative Solutions (Medoids)", x = var_1_label, y = var_2_label) +
      theme_minimal()
  } else if (num_variables_to_plot == 3) {
  ...
  cat("K-medoids analysis completed successfully!\n")
}
