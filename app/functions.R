...
check_align = function(inipath="../input/config.ini",var_path="../input/var_corr_par.csv"){
  inipath <- path_map_opt(inipath)
  var_path <- path_map_opt(var_path)
...
write_pca_ini <- function(var1 = "", var2 = "", var3 = "", var4 = "",
  ...
  # Ensure target directory exists (avoid 'cannot open the connection')
  dir.create(dirname(inipath), recursive = TRUE, showWarnings = FALSE)

  # Load existing config if present; otherwise start a fresh list
  config <- NULL
  if (file.exists(inipath)) {
    # Best-effort read; if it fails, fall back to empty list
    config <- tryCatch(read.ini(inipath), error = function(e) list())
  }
  if (is.null(config)) config <- list()

  # Ensure section [[5]] exists
  if (is.null(config[[5]])) config[[5]] <- list()
  ...
  # Best-effort write with clear warning instead of crashing observers
  tryCatch({
    write.ini(config, inipath)
  }, error = function(e) {
    warning(sprintf("write_pca_ini: failed to write '%s': %s", inipath, e$message))
    invisible(NULL)
  })
...
get_obj_range = function(filepath = "../data/pareto_fitness.txt",colnames=paste0("objective", seq(1, 4))){
  filepath <- path_map_opt(filepath)
  ...
  original_path <- path_map_opt("../data/pareto_fitness_original.txt")
  if(file.exists(original_path)){

    pf = read.table(original_path,sep=deli(original_path))
    colnames(pf) = colnames
...
pull_shp_new = function(layername = "hru", hru_in_opt_path="../input/hru_in_optima.RDS"){
  hru_in_opt_path <- path_map_opt(hru_in_opt_path)
  data_dsn <- path_map_opt("../data/")
  if(file.exists(paste0(data_dsn, layername, ".shp")) && file.exists(hru_in_opt_path)){
    hio= readRDS(hru_in_opt_path)

    hru = hio %>% filter(rowSums(!is.na(select(., starts_with("V")))) > 0) #only those hru that are used

    cm = sf::read_sf(dsn = data_dsn, layer = layername) #adapt path
